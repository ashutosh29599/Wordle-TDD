...
@task
def setup():
    sh('python -m pip install -U coverage parameterized radon tk')

@task
def test():
    sh('python -m coverage run --source src --omit="src/wordle_ui.py" -m unittest discover -s test')
